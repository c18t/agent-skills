...
（`issue-work` は issue 番号から始まるが、リリースには対応する issue が無い）。
...
ブランチ名は **`release/<プラグイン名>-<version>`**（例 `release/issue-flow-0.3.0`）。
...
⚠️ **version のドットはそのまま使う。** `issue-work` 手順 2 の「小文字英数字とハイフンのみ」は
issue 由来のブランチを想定した制約で、release ブランチは別カテゴリとして扱う。
git はドットを許容し（`refs/heads/release/issue-flow-0.3.0` は有効）、semver 表記のままの
...
release ブランチは**必ず新規**なので、`issue-work` 手順 3 のような有無の分岐は要らない。
...
⚠️ **起点を `origin/<デフォルトブランチ>` と明示する。** `issue-work` 手順 3 と違って
...
形式と細かい分岐は `issue-work` スキルの手順 4 と同じ。
...
実行コマンドはプロジェクトから判断する（`issue-work` 手順 7 と同じ表の見方）。
...
タイトルは `release: <プラグイン名> <version>` の形にする（例 `release: issue-flow 0.3.0`）。
...
スクリプト経由なら通る。判断の詳細は `issue-work` スキルの手順 11 にある。
