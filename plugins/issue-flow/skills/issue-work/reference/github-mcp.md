...
1. プラグインが有効か確認する（Claude Code なら `/plugin`、Cowork なら Customize → Plugins）。
   Python 3（`python3` / `python` / `py`）が `PATH` に無いとフックは起動しない
2. それでも動かないなら、**直前にファイル全文を Read してから**その内容を文字列で渡す。
   🔴 **記憶から書かない**
3. 下の照合を必ず取る
