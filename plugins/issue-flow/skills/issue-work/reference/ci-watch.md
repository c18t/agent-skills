...
このページの `gh` / `watch_pr.py` は gh 経路のもの。`gh` が無い環境（Cowork など）は
...
Windows（PowerShell）では `python.cmd` から同じ本体を起動する。
...
- 全 check の `bucket` を個別に調べ、`pending` が 1 件でもあれば待つ。配列そのものを
  判定すると常に真になり、**CI を待たずに素通りする**
...
`watch_pr.py` は `gh pr checks` 前提なので、`gh` が無い環境（Cowork など。経路の判定は
...
「差分だけを出す」「全 check の判定」はポーリングしないので関係しない。
