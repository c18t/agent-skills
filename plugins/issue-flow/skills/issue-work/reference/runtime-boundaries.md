# Codex で worktree の作業境界を守る

`issue-work` と `release-merge` で共用する。`EnterWorktree` / `ExitWorktree` が公開されていない
Codex では、チャット全体の cwd や全ツールの書き込み境界を worktree へ移す API があると仮定しない。
固定配置、絶対 `workdir`、操作直前の preflight で Claude Code の隔離に相当する境界を作る。
...
`<branch-slug>` はブランチ名のスラッシュをハイフンに置換したもので、`issue-work` の
`<接頭辞>/<番号>-<スラッグ>` も `release-merge` の `release/<プラグイン名>-<version>` も同じ規約に
従う。🔴 **リポジトリ外の sibling パスへ作らない**（writable root を外れて追加の権限承認が要る）。
...
`release-merge` の release → main も同じく main checkout から行う（PR 同士のローカル merge までは
worktree 側）。
