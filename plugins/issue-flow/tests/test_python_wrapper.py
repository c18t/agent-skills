...
このラッパーは notion-writeback のものと同一実体（プラグインは互いの
CLAUDE_PLUGIN_ROOT を参照できないので複製している）。テストも合わせて持つ。

`python3` をコマンド名として決め打ちすると Windows で起動できず、
@@FILE: センチネルが未展開のまま GitHub へ push される（notion 側 #2 と同じ壊れ方）。
