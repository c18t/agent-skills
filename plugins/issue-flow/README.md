...
2. Names the branch by asking whether the change splits along plugin boundaries — not whether the plugins depend on each other. A change that stays inside one plugin gets `release/<plugin>-<version>`, where the plugin name is mandatory in a monorepo of independently versioned plugins and the version's dots are kept so the branch matches the tag. A change that crosses boundaries gets one `release/merge-<date>` instead: enumerating plugins and versions passes 80 characters at three plugins, drags the worktree path toward Windows' 260-character `MAX_PATH`, and goes stale the moment a version is revised — so the inventory lives in the PR body, which has room for it
3. `git worktree add` from `origin/<default>` at `.claude/worktrees/<branch-slug>` — the same placement `issue-work` uses — registers it in `*.code-workspace`, and **`EnterWorktree`**; in Codex it fixes the absolute `workdir` and runs the same root/branch preflight. Conflict resolution belongs in its own tree, not in the main checkout it would otherwise hold hostage
...
- Both worktree skills place their tree under `.claude/worktrees/`, which is Claude Code's own default location, so `EnterWorktree` normally needs no approval prompt (`issue-work`, `release-merge`)
...
- The worktree is created manually under `.claude/worktrees/`, so cleanup remains an explicit `git worktree remove`; Claude Code's sweeper leaves manually created worktrees alone. Both skills use that one placement: a sibling path outside the repository falls outside Codex's writable root, so even a temporary directory next to the tree then needs an extra approval
